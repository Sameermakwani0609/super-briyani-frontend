        ...
        {orders.length === 0 ? (
          <div className="text-center">
            ...
            <p className="text-gray-400 mb-8 text-sm sm:text-base">
              Your orders for today will appear here.
            </p>
          </div>
        ) : (
          <div className="max-w-3xl mx-auto space-y-6">
            {orders.map((order) => {
              const createdStr = order.createdAt?.toDate
              ...
              return (
                <div
                  key={order.id}
                  className="bg-gray-900 rounded-lg p-4 sm:p-6 border border-yellow-500"
                >
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-2">
                    ...
                    <span
                      className={`px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-semibold ${statusClass}`}
                    >
                      ...
                      {(order.items || []).map((i, idx) => {
                        ...
                        const unitDisc =
                          i.unitDiscountedPrice != null
                            ...
                            : Math.max(0, unit * (1 - snapshotPct / 100));
                        const lineBase =
                        ...
                        const lineDisc =
                          i.lineDiscountedTotal != null
                            ...
                            : unitDisc * qty;
                        const hasDisc = lineDisc < lineBase - 1e-6;
                        return (
                          <div
                            key={idx}
                            className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gray-800 p-2 sm:p-3 rounded"
                          >
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 mb-4">
            ...
            <button
              onClick={() => {
                ...
                setHistoryResults(
                  allOrders.filter((o) => matches(o.createdAt))
                );
              }}
          ...
          {historyResults.length > 0 && (
            <div className="space-y-4">
              {historyResults.map((o) => {
                const createdStr = o.createdAt?.toDate
                ...
                const finalTotal =
                  o.discountedTotal != null
                    ? Number(o.discountedTotal)
                    : (o.items || []).reduce(
                        (t, it) =>
                          t +
                          (it.lineDiscountedTotal != null
                            ...
                            : Math.max(
                                ...
                                Number(it.price || 0) *
                                  (1 -
                                    Number(o.appliedDiscountPercent || 0) /
                                      100) *
                                  Number(it.quantity || 0)
                              )),
                        0
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-2 sm:gap-3 text-xs sm:text-sm text-gray-300">
                      <div>
                        <span className="text-yellow-400">Name:</span> {o.name}
                      </div>
                      <div className="text-right md:text-left">
                        <span className="text-yellow-400">Total:</span>
                        {hasDisc ? (
                    ...
                    <div className="mt-3 space-y-1">
                      {(o.items || []).map((i, idx) => {
                        ...
                        return (
                          <div
                            key={idx}
                            className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gray-800 p-2 rounded text-xs sm:text-sm text-gray-300"
                          >
                            <div>
